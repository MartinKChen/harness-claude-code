...
Read the slice issue #<n> body. Locate the task block(s) for <ids> in the `## Tasks` checklist (each entry is a header `[ ] \`<id>\` · **<type>** · blocked-by <ids|—>`, the delivery on the next line, a metadata line tagging `covers:` (AC clause ids) + the type pointer `contract:`/`entry-source:`/`done:`, then `scenario:` and a fenced ```gherkin block of named Scenarios walked at this task's owning layer). The checklist is the durable task ledger — a box already checked `[x]` means that task is DONE; skip it (resume safety). Read the slice's Acceptance criteria (EARS) and the task's own `scenario:` Gherkin for behavior; the task's owning layer is its `type` (backend → HTTP endpoint/worker, frontend → rendered tree) — write the acceptance test there (see `principle-engineer-tdd`). Follow the pointer (api-contract / data-model / the task's `scenario:` Gherkin / design tokens) for the unit spec.
...
- **The slice's already-authored E2E specs — `frontend` tasks only, read-only.** The `e2e` tasks are sequenced ahead of `frontend` (the first `frontend` task is `blocked-by` the last `e2e` task), so by the time a `frontend` task is dispatched its specs are already committed on the slice branch. Before building the UI, list and read them (three-dot, merge-base scoped, so specs that landed on main after this branch forked are not mistaken for the slice's own): `git diff --name-only origin/main...HEAD -- '**/e2e/**/*.spec.*' | sort -u`. Read them as the **affordance/interface reference only** — the accessible names, ARIA roles, semantic landmarks, and navigation path the specs *query* — so the UI you build exposes the affordances the pass-E2E gate will look for, instead of a coin-flip mismatch discovered late after a full stack boot. **The Gherkin AC + your own acceptance test remain the behavioral oracle** — derive *what is true after the flow* from the AC, never from the spec. A spec that asserts behavior the AC never called for is a divergence to surface (it bounces at the pass-E2E gate), **not** something to implement just to make the spec pass. Never modify the specs here.

- **Contracts are read-only; halt on a gap at implement time.** The `contract:` pointers (`docs/api-contract/*`, `docs/data-model/*`) are specs you build against, never files you edit. The moment implementing a task reveals the contract is wrong, missing an outcome (e.g. a status code the handler must return), or contradictory, STOP — do not invent behavior and do not patch the contract. Flip the slice to `status:need-attention` with a `# Contract change requested` comment and exit. Catching it here, before review, is the cheapest place.
