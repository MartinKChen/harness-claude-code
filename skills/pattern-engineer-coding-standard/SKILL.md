...
- **The contract files are READ-ONLY to you.** `docs/api-contract/*` and `docs/data-model/*` are inputs you implement against — never outputs you edit. If the contract is genuinely wrong, missing a status code/field, or self-contradictory, that is an architect decision: do not patch it — halt to `status:need-attention` with a contract-change request (see the `workflow-engineer-*` lanes).
