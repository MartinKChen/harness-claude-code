...
- **Spec pointer** — each task's checklist follow-on line tags, **uniformly across all three types**, the AC clause(s) it discharges and the scenario it walks at its layer; it does NOT duplicate the AC text. The slice-level AC is EARS only; Gherkin scenarios are written **per task**, not in an upfront slice-level block.
  - `covers:` (every type) — the AC clause id(s) this task discharges (e.g. `AC1, AC3`). This is what lets the reviewer tick an AC box once every task tagged with it passes — **and** the closed set the pre-implementation E2E coverage gate computes (it gates against ⋃ `covers:` over the slice's `e2e` tasks, NOT the full slice AC set). So an `e2e` task's `covers:` MUST list **every AC whose user-visible / journey clause that walk actually traverses and asserts**, not only the AC that earned the task. When a journey-closing E2E walk passes through the rendered state of another AC — e.g. the magic-link walk for AC1/AC4 also renders the confirm-prompt page that is AC2's user-visible clause — that AC's E2E-observable clause is discharged there, so list it (`covers: AC1, AC2, AC4`). Omitting it silently drops that AC out of the E2E gate's closed set. (This does **not** contradict "a two-layer AC does not earn a browser walk" above: you are not creating a *new* e2e task for the compound AC — you are recording that an *already-earned* journey walk also traverses its user-visible clause.) Conversely, never list a backend/frontend-owned AC on an `e2e` task whose walk does not actually assert that AC's user-visible state.
...
- **Tasks carry `covers:` + `scenario:` + a spec pointer, not their own AC.** AC lives on the slice as EARS only; Gherkin lives per task. Each checklist task's follow-on line tags, uniformly across types, `covers:` (the AC clause ids it discharges) and `scenario:` (its own Gherkin block, walked at its owning layer — there is no upfront slice-level Gherkin block), plus a type-specific pointer: `backend` → `contract:` its `docs/api-contract/<entity>.yaml` (+ `docs/data-model/<entity>.yaml` when it introduces the entity); `e2e` → the mapped journey scenario + the critical-path Journey step it realizes; `frontend` → `design:` tokens (pages also `entry-source:`/`reached-from:`). The one exception is a **contract-less utility** task, which carries a `done:` one-line criterion in place of `contract:` (still with `covers:`/`scenario:`) — the only place a task states its own AC.
