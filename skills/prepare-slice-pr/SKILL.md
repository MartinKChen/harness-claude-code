...
| `scripts/push-create-pr-clear-prepare.sh <slice-#> <slice-branch> <title> <body-file> [--milestone <name>]` | Push the slice branch, open the draft PR, and remove `status:prepare-pr` from the slice. Terminal success action. |
...
Inputs from the orchestrator: a slice issue number. The orchestrator (`create-draft-pr`) already labeled the slice with `status:prepare-pr` as its lock and confirmed every task sub-issue is closed and no PR exists on the slice branch — do not re-check those pre-conditions. Everything else (slice branch, milestone, task sub-issue numbers, touched E2E specs, runtime command) you discover yourself.
...
Capture `milestone.title` for step 7 (open-draft-pr) and the slice's `title` for the PR title. If the slice has no milestone, the PR opens without one — never fabricate.

Resolve the slice branch and the list of closed task sub-issue numbers — both are reused in step 6 (PR body composition) and step 7 (push + open PR):
...
### 5. Identify and run every E2E spec touched on the slice branch
...
Capture the full test output to a temp file — both the success-path summary and any failure details. The output is the input to step 5a's triage.

#### 5a. Triage each failure
...
Continue to step 6 (compose PR body) only after every touched spec is green and the audits are clean.

#### 5c. Bail out — flip the slice to `status:need-attention`

When the triage in step 5a classifies one or more failures as E2E-spec bugs, compose a diagnostic comment that lists every failure (production-code + spec) with:
...
The script removes `status:prepare-pr` from the slice, adds `status:need-attention`, and posts the comment. Stop immediately after the script returns — do not push any partial fixes, do not open a PR, do not loop. The user reviews the diagnostic and either rewrites the spec(s) or pushes a fix themselves, then clears `status:need-attention` so `create-draft-pr` can re-pick the slice on the next fire.

### 6. Compose the PR body

Reached only when every touched E2E spec is green (or the slice had no touched specs and step 5 was a no-op). Start from `templates/pr-body.md`, which ships the standard PR-body sections (What / Why / How / Testing / Screenshots / Checklist) plus a trailing linked-issues block with three placeholders: `<slice-#>`, `<task-#-1>` … `<task-#-N>`.
...
Copy the template, fill in the prose sections (or drop sections that don't apply), then replace the trailing-block placeholders with the actual slice and task numbers — write the final body to a temp file for `push-create-pr-clear-prepare.sh`:
...
### 7. Push, open the draft PR, and clear `status:prepare-pr`
...
The script pushes the slice branch (the pre-push hooks re-run the fullstack lint/format/type/test set and the security scans against the worktree — drop back into step 5b if any hook denies; never force-push, never skip hooks), opens the draft PR with the body file and milestone, and removes `status:prepare-pr` from the slice issue.
...
- **`status:prepare-pr` is the lock — the engineer removes it on every terminal path.** Either step 7's success path (push + open PR + remove label) or step 5c's bail path (remove label + add `status:need-attention`) clears it. Leaving `status:prepare-pr` on a slice after exit would prevent `create-draft-pr` from ever re-picking it.
