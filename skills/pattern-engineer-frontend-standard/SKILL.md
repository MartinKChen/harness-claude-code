...
- **A registered route is not a reachable page — the invariant is reachability.** The task's declared **Entry source** (carried from `docs/design-system/surfaces.md` onto the task body) must exist as a real inbound path in code, and a test must exercise it:
...
- **Ship a contract-conformance test for the surface — the executable firewall.** Render the surface in a component test (RTL) and assert every declared `region` + `action` resolves by `getByRole(role, { name })`, plus each `states` entry this slice ships. Without it, contract conformance is only eyeballed at review and drifts; with it green, every E2E locator built from the contract's role+name is *guaranteed* to resolve — the locator-mismatch churn moves out of the brittle browser layer into a fast component test. The deletable line is the `role`/`aria-label`/labelled-control producing each accessible name: delete it and this test must fail.
...
- **Render to the behavioral `states` the spec locked; extend only what it didn't.** For any outcome an E2E spec asserts, the E2E author has already locked the matching `states` entry in `docs/ui-contract/<screen>.yaml` (the queryable proof) — render to it *exactly*; do NOT redefine it (a divergence is a one-way code→contract fix, asserted by the conformance test and gated at pass-E2E, so the behavioral outcome can't drift from the spec). For a frontend-only behavior NO E2E covers (a state only an RTL test proves — e.g. a transient client-side toggle), YOU add the matching `states` entry in the same commit set, naming the queryable proof: the `status`/`alert` role+name, the `field_errors` message tied to its field, the `redirects_to` route. Either way the reviewer clears a state by its being declared + rendered + asserted (conformance test), never by an undocumented render — build the UI to match what the contract declares.
...
- The surface's `docs/ui-contract/<screen>.yaml` is your **published affordance source** (see "UI interaction contract" above). The slice's already-authored E2E specs are the **secondary cross-check**: read them read-only and confirm your rendered interface matches the names/roles/click-path they query (the affordance reference when no UI contract exists). Interface-matching only — the Gherkin AC stays the source of *what the UI must do*; a spec asserting behavior the AC never specified is a divergence to surface at the pass-E2E gate, not something to bake into the UI.
