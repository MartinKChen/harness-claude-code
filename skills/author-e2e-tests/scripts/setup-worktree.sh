...
# caller must invoke `handle-rebase-conflict.sh` to surface the diagnostic and
# stop the author run.
#
# Usage:
...
if ! git -C "$worktree_path" rebase origin/main; then
  ...
  echo "rebase of ${slice_branch} onto origin/main hit a conflict — invoke handle-rebase-conflict.sh" >&2
  exit 1
fi
