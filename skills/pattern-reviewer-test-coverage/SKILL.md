...
- **One MEDIUM finding per gap.** All coverage gaps are MEDIUM (reported, but informational — the `reviewer` agent only blocks on CRITICAL / HIGH). Never inflate to HIGH ("AC is critical so it should be HIGH") — coverage is coverage.
...
Walk the patterns in order. Collect findings as `{title, severity (MEDIUM), location (test_file_path or "missing — should live at <suggested_path>"), AC/scenario reference, gap (what is not asserted), fix (concrete suggestion)}` records — the agent composes the final comment.

### AC coverage from `## Done criteria (EARS)` — every `type:*`

For every AC in the task body (`AC1`, `AC2`, …):

- **AC not exercised** — no test in the diff whose description names the AC's behavior and whose assertions check the `SHALL` / `MUST` / `THEN` clause. File a MEDIUM finding citing the AC label + the test file that should have covered it.
- **Shallow coverage** — a test exists but asserts only the happy-path return value and skips the `IF <condition>, THEN …` branch in the same AC, or only the `MUST` clause and skips the `And it SHOULD …` clause when that secondary response is observable. File a MEDIUM citing the missed clause.
- **Wrong layer** — an AC about an HTTP endpoint covered only by a pure-function unit test (the request/response contract is never exercised), or a frontend AC about user-visible state covered only by a hook test that never renders the component. File a MEDIUM citing both layers (where it's covered, where it should be covered).
...
### Scenario coverage from `### Scenarios (Gherkin)` — every `type:*`

For every `Scenario:` block in the task body (or in the parent slice body for `type:e2e`):

- **Scenario not exercised** — no test walks the `Given → When → Then` sequence. File a MEDIUM citing the scenario name.
- **Partial scenario** — a test covers Given + When but never asserts the Then (or asserts a different Then than the spec). File a MEDIUM citing the missed assertion.
...
### Migration scenario coverage — only when `### Migration scenarios (Gherkin)` is present
...
- **Missing migration test** — the diff must include a migration test (e.g., `pytest-alembic` upgrade/downgrade) that walks both upgrade and downgrade. Missing either side is a MEDIUM gap.

### Edge-case breadth — `type:backend` / `type:frontend` only
...
For each edge that applies but is missing, file one MEDIUM finding. Consolidate — if five new functions all miss boundary-value coverage in the same way, file one consolidated finding listing all five functions.
...
### Selector + assertion quality — `type:e2e` only
...
- **Selector quality** — semantic selectors (`getByRole`, `getByLabel`, `getByPlaceholder`, accessible name) preferred over `data-testid`. `data-testid` is acceptable only when there is no semantic anchor; the test must justify the exception inline. Brittle CSS-class / XPath selectors are MEDIUM.
- **Assertion quality** — assertions check user-visible state (text, role, visible class, URL), not raw HTTP responses or DOM internals. A spec that only awaits a network response without asserting the rendered UI is a MEDIUM coverage gap (the user-visible outcome is unverified).
- **One critical-path flow per spec** — do not bundle independent flows into a single spec. A spec that walks login + checkout + profile-edit in one `test()` is MEDIUM (failure isolation breaks).
- **Stylistic issues** in test files (variable naming, mixed quotes) are LOW.
...
```markdown
### [MEDIUM] <AC/scenario label> — <one-line title — no leading `#N`>
...
- Severity is always `MEDIUM` for coverage findings.
