...
description: "Reviewer lens for the test-coverage gate on any `type:*` task or slice. The catalogue of *what counts as a complete test set* lives in `pattern-test-coverage` (shared with the engineer); this skill adds the reviewer's verb — how to turn a gap into a finding: a gap that maps to a declared AC / Gherkin / contract clause on a touched surface is HIGH and blocks the gate, while reviewer-judgement edge breadth that no spec clause names (and any pre-existing gap) is a non-blocking Defer/Nit; cite the AC/scenario label AND the test file that should have covered it, use a non-numeric handle, read surrounding code before flagging, and emit findings in the `# Code Review` comment shape for the dispatching reviewer agent to compose. Activate on every code-gate review."
---
...
- **The slice body is the spec, and each AC has an owning layer.** Coverage is judged against the slice body's `## Acceptance criteria (EARS)` + `### Scenarios (Gherkin)` blocks (and `### Migration scenarios (Gherkin)` for data-model tasks), and each task's `covers:` (AC clause ids) + `scenario:` follow-on line. Judge each task at **its owning layer** (its `type`: `backend`→HTTP endpoint/worker, `frontend`→rendered tree, `e2e`→browser): the `covers:` clause must be discharged there, the `scenario:` walked there. The completeness bar is the **deletable-code lens, not AC→test count** — one AC may need several tests; several ACs may ride one walk.
