...
description: "Reviewer lens for the test-coverage gate on any `type:*` task or slice. The catalogue of *what counts as a complete test set* lives in `pattern-test-coverage` (shared with the engineer); this skill adds the reviewer's verb — how to turn a gap into a finding: every coverage gap is HIGH and blocks the gate, cite the AC/scenario label AND the test file that should have covered it, use a non-numeric handle, read surrounding code before flagging, and emit findings in the `# Code Review` comment shape for the dispatching reviewer agent to compose. Activate on every code-gate review."
---
...
- **The task body is the spec.** Coverage is judged against the task issue body's `## Done criteria (EARS)` and `### Scenarios (Gherkin)` blocks (and `### Migration scenarios (Gherkin)` for data-model tasks). For `type:e2e` tasks, the canonical scenario text is in the **parent slice issue** body.
...
- **Every coverage gap is HIGH and blocks the gate.** Issue-body criteria (AC, `Scenario`, `Migration scenario`), edge-case breadth (boundary, error, empty, concurrency, idempotency, authz), named-observable assertions, emitted-artifact correctness, and `type:e2e` selector + assertion quality are all HIGH. Tests are the safety net; a hole anywhere is a HIGH gap, whether spelled out in the issue body or surfaced by reviewer-judgment edge breadth. The only LOW carve-out is purely stylistic noise in E2E test files (naming, mixed quotes) with no effect on what the spec covers.
...
Walk `pattern-test-coverage` §1–§7 against the scoped diff. For each gap, collect a record:
...
Severity is **HIGH** for every coverage gap the catalogue defines — issue-body criteria, edge-case breadth, named-observable assertions, emitted-artifact correctness, and E2E selector/assertion/bundled-flow gaps. **LOW** is reserved for purely stylistic E2E noise (naming, mixed quotes). Consolidate repeats — if five new functions all miss boundary coverage the same way, file one finding listing all five.
