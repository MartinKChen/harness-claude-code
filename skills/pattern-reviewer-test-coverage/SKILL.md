...
- **When a diff-scoped mutation report exists, read it — don't re-derive the deletable-code lens by eye.** If the worktree / CI run carries a surviving-mutant report (`mutation-report.txt`, from `scripts/mutation.sh` — Stryker `--since` / mutmut over the branch diff), it is the *deterministic* discharge of the deletable-code spine: a surviving mutant on a line that maps to a manifest AC's `SHALL`/`THEN` clause is a concrete HIGH coverage gap — cite the mutant (file:line + what was flipped) instead of arguing adequacy from inspection. A survivor on a changed line that no spec clause names is Deferred/Nit like any other reviewer-judgement edge. False-positive guard: a provably **equivalent** mutant (the flipped code is semantically identical to the original — no input can distinguish them) is not a gap at all; its fix is the tool's exemption (`# pragma: no mutate` / the score threshold), never a manufactured test that asserts nothing. The report supplements the catalogue walk (it can't see selector-collision, named-observable, or owning-layer errors); absence of the report is not a pass — fall back to the manual lens.
