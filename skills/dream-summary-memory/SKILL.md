...
**Hard-gate candidates.** When a kept pattern is *deterministically enforceable* by a lint / type / format rule — a tool could catch every instance with zero judgment (e.g. `== None` → ruff `E711`; un-awaited coroutine → ruff `ASYNC` / `RUF006`; a floating promise → biome `noFloatingPromises`; a class of bug a missing `tsconfig` strictness flag let through) — it is a candidate to **graduate from a prose overlay into the project's hard gate** (`pyproject.toml` `[tool.ruff.lint] select`, `biome.json` rules, or `tsconfig.json`). A hard gate kills the whole *class* deterministically on every push, instead of relying on the agent remembering to read the overlay — which is the strongest possible form the rule can take. But promoting a gate is **repo-wide and disruptive**: it can fail every open PR at once, and it must move in lockstep with the scaffold template and the matching `pattern-engineer-*` skill. So dream-summary **recommends; it does not edit the gate config**. Write the soft overlay rule as usual (so the rule is enforced immediately, if softly) AND record a hard-gate candidate per §5 for a human — or a follow-up engineer task — to action deliberately.
...
- **Hard-gate candidates** (per §4) → append under the overlay's `## Hard-gate candidates` section, naming the exact change: the tool, the rule code / config key, the target config file (`pyproject.toml` ruff `select`, `biome.json`, or `tsconfig.json`), the recurrence count + issue/PR refs, and one BAD snippet from the history. **Do NOT edit `pyproject.toml` / `biome.json` / `tsconfig.json` (or any gate config) yourself** — these are repo-wide gates whose promotion is a reviewed, deliberate act outside dream's autonomous-write mandate. Surface every hard-gate candidate prominently in the run report and the `dream-log.md` entry so a human can action it.
...
- **Recommend hard gates; never flip them.** When a recurring, mechanically-checkable mistake should become a lint/type/format hard gate, record it as a `## Hard-gate candidates` recommendation naming the exact rule + config file. NEVER edit `pyproject.toml`, `biome.json`, `tsconfig.json`, or any other gate config: a hard gate is repo-wide and can break every open PR, so its promotion is a deliberate, human-reviewed change kept in lockstep with the scaffold template and the matching `pattern-engineer-*` skill. Autonomous writes stay confined to additive overlays + the log.
