...
- **The code-quality polish is the one CAPPED loop — exactly one cycle.** Because code-quality findings don't block, the slice never churns on them: after the gating `APPROVE`, a single `engineer` fix runs over the non-gating findings followed by one re-review (a `qualityPolished` flag enforces the cap; a gating regression the polish introduces is converged away by the normal uncapped BLOCK path, never re-polished). The non-gating findings still standing after that one pass are filed as `kind:enhancement` issues — one per review dimension, each a per-finding checklist (deduped against open enhancements) — for the `/ship` maintenance lane, rather than holding the slice open.
