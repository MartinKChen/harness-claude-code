...
5. **Define the per-task dispatch contract.** There is no persistent agent team in this skill — instead, every unblocked task is dispatched as a one-shot, unnamed sub-agent via the `Agent` tool. The `subagent_type` is selected from the originating sub-issue's **`type:<type>` label** (captured in step 2), and the role for `engineer` sub-agents is passed explicitly in the dispatch prompt (the `engineer` agent does not infer its role from a name):
   ...
   |-----------------|-----------------|-------------------------------------|
   | `type:e2e`      | `e2e-author`    | n/a (the agent is already E2E-scoped) |
   ...
   - For `e2e-author` sub-agents (no `engineer` role): the agent runs its own discovery + per-task worktree workflow per `agents/e2e-author.md`; the orchestrator just hands it the task issue ID, title, and URL.
   ...
   For e2e-author: cut your own per-task worktree off the parent slice's branch per
     agents/e2e-author.md, do your work there, then rebase + push back.
...
8. **Drive tasks to completion.** Loop until every task is `completed`:
   ...
   - For each such task, spawn one one-shot sub-agent via `Agent` per the dispatch contract in step 5 — independent unblocked tasks go out in parallel as multiple `Agent` calls in the same message. Each engineer sub-agent receives its role (backend or frontend) explicitly in the prompt; each `e2e-author` is dispatched without role text.
...
10. **Spin up a validation Agent Team and gate merge on security → code review.** Validation runs as a persistent Agent Team — created via `TeamCreate` — with four **named** members so they can hand off to each other via `SendMessage` rather than routing every round-trip through the orchestrator. **Do NOT call `TaskCreate` for this phase.** The orchestrator pings `security-reviewer` to review the PR, then pings `code-reviewer` to review the PR, and only advances to step 11 once **both** reviewer members have reported clean. The Playwright E2E suite is validated in parallel by a GitHub Actions workflow against the PR — this skill does not run E2E itself, and merge-readiness also requires that workflow to be green (a check the human merger enforces in step 11).
    ...
    | `code-reviewer`      | `code-reviewer`     | n/a (read-only — never edits code itself)    |

    Every member's spawn prompt MUST also include: the names of the other three teammates, an explicit instruction that they can address each other directly via `SendMessage` (e.g. `security-reviewer` hands a fix to `backend-engineer` without going through the orchestrator), the parent feature-worktree path + `feature/<branch-name>`, and the PR URL.
    ...
    - `security-reviewer` and `code-reviewer` work directly in the parent feature worktree on `feature/<branch-name>`.

    Run validation in two ordered phases — **security first, code review second**. Do NOT advance to step 11 until `security-reviewer` AND `code-reviewer` have both reported clean to the orchestrator against the same commit.

    - **a. Security review (first).** From the orchestrator, `SendMessage` to `security-reviewer` with the PR URL and diff scope; ask it to validate the implemented changes against the `security-patterns` checklist. For every failing pattern, `security-reviewer` hands the fix off to `backend-engineer` or `frontend-engineer` via `SendMessage` (the reviewer is read-only and never edits code itself). After fixes land, `security-reviewer` re-validates. The orchestrator waits for `security-reviewer` to report a final clean pass. Do NOT proceed to phase b until then.

    - **b. Code review (second, only after security is clean).** From the orchestrator, `SendMessage` to `code-reviewer` with the PR URL and diff scope; ask it to walk the code-review checklist (security → code quality → framework patterns → performance → best practices) and dispatch findings via the PR-comment + `SendMessage` flow defined in `agents/code-reviewer.md`. For every >80%-confidence finding, `code-reviewer` hands the fix off to `backend-engineer` or `frontend-engineer` via `SendMessage` (the reviewer is read-only and never edits code itself). After fixes land, `code-reviewer` re-validates, and — if any code changed — also pings `security-reviewer` via `SendMessage` to re-run its gate so both gates remain clean against the same commit. The orchestrator waits for `code-reviewer` to report a final clean pass (no CRITICAL or HIGH findings outstanding — verdict `APPROVE`); if `security-reviewer` had to re-run, also wait for it to report clean again. Only when **both** reviewer members have reported clean does control return to step 11.

11. **Push the post-validation code and comment on the PR.** Once both the security review and the code review are clean, defer to `git-workflow` to push the new commits made during step 10 to the existing remote branch `feature/<branch-name>` so the open PR picks them up automatically. **Do NOT edit the PR description.** Instead, post a comment on the PR (e.g. `gh pr comment <pr-number> --body …`) recording the validation outcome — at minimum: any security findings + their fix commits, any code-review findings + their fix commits (and final verdict), and the head commit SHA both gates were clean against. The comment is the durable audit trail; the PR body stays as it was opened in step 9. Re-share the PR URL with the user with a one-line note that the security review and code review are complete and that the GitHub Actions E2E workflow is the remaining merge gate.

12. **Tear down the validation team and the local workspace.** Once the PR is open, up to date, and `security-reviewer` and `code-reviewer` have both reported clean: first call `TeamDelete` to stop the validation team created in step 10 (`backend-engineer`, `frontend-engineer`, `security-reviewer`, `code-reviewer`). Next, from inside the parent feature worktree at `<absolute-worktree-path>`, run `docker compose down -v` to stop the validation stack and drop its volumes so no leftover containers, networks, or named volumes linger after the worktree is gone. Then, from the main repo checkout, defer to `git-workflow` to run `git worktree remove <absolute-worktree-path>` (use `--force` only if the worktree has uncommitted state the user has already approved discarding) and then `git branch -D feature/<branch-name>` to delete the local branch. The remote branch `origin/feature/<branch-name>` stays — the PR still needs it. Confirm each step succeeded before continuing; if any fails (e.g. uncommitted changes, worktree locked, compose teardown errors), surface the error to the user instead of forcing.
...
- **E2E authoring leads implementation; E2E validation runs in CI.** The `e2e` sub-issue (dispatched as an `e2e-author` in step 8) blocks every implementation task — `create-issues` already encoded this via the E2E-first rule, so the Playwright specs land before any backend/frontend work begins. The full E2E suite is then validated by a GitHub Actions workflow on the PR — this skill does not run E2E itself, and step 10 does not include an E2E gate. For backend-only main issues with no `e2e` sub-issue, the implementation-side `e2e-author` dispatch is skipped.
- **Validation order is fixed: security first, then code review.** In step 10, the orchestrator pings `code-reviewer` only after `security-reviewer` reports clean. If code review surfaces fixes, security re-runs after the engineer-team fixes land — `security-reviewer` AND `code-reviewer` must both be clean against the same commit before step 11 starts.
...
- **Code review is mandatory before declaring done.** The PR is not "ready" until a `code-reviewer` sub-agent reports a clean pass — verdict `APPROVE` with zero CRITICAL and zero HIGH findings outstanding — and any fixes have been pushed to the PR branch.
- **`security-reviewer` and `code-reviewer` are read-only.** All code changes during the security and code-review loops are made by `engineer` sub-agents dispatched per fix; the reviewers only validate and report findings (security against the `security-patterns` checklist; code against the agent's own checklist + four-question audit).
...
- **All sub-agents run in `mode=auto`.** Every sub-agent dispatched by this skill (`engineer`, `e2e-author`, `security-reviewer`, `code-reviewer`) is spawned with `mode=auto` so it executes autonomously without prompting back on routine decisions. Each sub-agent is also instructed to pass `mode=auto` to any nested `Agent` calls it makes, since the permission mode does not propagate down nested `Agent` calls automatically.
...
1. `#<sub-issue>` — type from `type:<type>` label — <sub-issue title>
   - Dispatched as: `subagent_type=e2e-author` | `subagent_type=engineer` (role: backend engineer) | `subagent_type=engineer` (role: frontend engineer)
...
**Validation phase (step 10):** spins up a named Agent Team — `backend-engineer`, `frontend-engineer`, `security-reviewer`, `code-reviewer` — that runs after the PR opens. No tasks are created for it. The orchestrator pings `security-reviewer` first via `SendMessage`, then pings `code-reviewer`, and proceeds to step 11 only when both reviewer members report clean. The Playwright E2E suite is validated separately by a GitHub Actions workflow on the PR. The team is torn down via `TeamDelete` in step 12.
