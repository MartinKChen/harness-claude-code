...
| `scripts/setup-worktree.sh <slice-branch> [--rebase-onto-main \| --merge-main]` | Create-or-reuse the worktree at `/tmp/harness-claude-code/<repo>/worktrees/<slice-branch>`, fetch + hard-reset to `origin/<slice-branch>` (always — agents always enter on the latest remote tip), then optionally integrate `origin/main`. `--rebase-onto-main` rewrites the branch onto main (NOT push-safe — needs a force-push; aborts on conflict, exit 2). `--merge-main` merges `origin/main` INTO the slice branch with an explicit merge commit (push-safe — no history rewrite, no force-push; on conflict it leaves the conflicted worktree for the caller to resolve+commit+push and exits 3). The two modes are mutually exclusive. Prints the worktree path on stdout (even on a merge conflict, so the caller can `cd` in to resolve). |
