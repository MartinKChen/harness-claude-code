...
| `scripts/create-enhancement.sh --title <t> --body-file <p> --intent <kebab> [--milestone <m>]` | Create one `kind:enhancement` issue (+ `status:ready-to-review`) from a feature-shaped body and link an `enhancement/<n>-<intent>` branch via `gh issue develop` — the single-issue analog of a create-feature-issues slice. Prints `issue:<n>` + `branch:<…>`. Called by the `create-enhancement-issue` skill and by the review-debt triage (`non-functional` findings). |
| `scripts/create-refactor.sh --title <t> --body-file <p> --intent <kebab> [--milestone <m>]` | Create one `kind:refactor` issue (+ `status:ready-to-review`) from a behavior-preserving body and link a `refactor/<n>-<intent>` branch via `gh issue develop` — the sibling of `create-enhancement.sh` for code-quality debt. Its `## Tasks` checklist carries NO `e2e` tasks and NO acceptance criteria, so `implement-slice` skips all E2E machinery. Prints `issue:<n>` + `branch:<…>`. Called by the review-debt triage (non-`non-functional` code-quality findings). |
