...
Task reviews load only the test-coverage gate — the **Phase 1 (spec compliance)** pattern: the shared `pattern-test-coverage` catalogue read through the `pattern-reviewer-test-coverage` lens. Phase 2 (code quality) does not apply at the task level; quality patterns are layered in at the slice level instead, where the diff is broader and the cross-task seams are visible. See `workflow-reviewer-review-slice` step 3 for the two-pass split that drives slice reviews.

The `pattern-reviewer-test-coverage` lens emits raw findings as `{title, severity, location, evidence, fix}` records — `severity` is the pattern's per-rule judgement (CRITICAL / HIGH / MEDIUM / LOW). Collect all of them; do not post per-pattern.
