...
- **Commit the file, not the file you assume git already has.** A passing local suite does not prove a file is committed — your test runner, and the pre-push hook, read the worktree from disk, so a file you authored but never `git add`-ed stays green locally and only fails on a clean CI checkout. Never assume a module is pre-existing because it's on disk: a GREEN step that introduces a new file must `git add` that path, and after every commit `git status --porcelain` must be empty for the work you just did. Stage the paths you touched explicitly if you like, but verify the result — `git status` clean is the proof, not "the tests pass."
