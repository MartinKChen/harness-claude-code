...
- **Assert on the contract's declared `states` — and when authoring pre-implementation, LOCK the state you assert.** A spec's outcome assertion targets a state the contract declares — the `status` / `alert` role+name, a `field_errors` message, the `redirects_to` route. The E2E author authors *before* the engineer builds, so for an outcome the contract doesn't yet declare, the author **adds the `states` entry to `docs/ui-contract/<screen>.yaml` as part of authoring the spec** (naming the queryable proof) — that locked entry is the fixed target the engineer renders to and the conformance test asserts, which is what stops the behavioral outcome drifting between spec and build. Declaring an assertion surface is within the test-author remit; it is not production code.
...
- **No invented roles or names — skeleton extension is the owner's job; `states` are the one carve-out.** If the contract doesn't declare a control, the spec doesn't query it. When the journey needs a **region/action** the skeleton lacks, that's a contract gap (`design-lead` owns the skeleton) — surface it as a question, don't paper over it with an off-contract locator. **`states` split differently:** the E2E author locks the states its specs assert (previous bullet); the engineer adds only frontend-only states no E2E covers.
...
- **Asserting an undeclared state.** Asserting on a toast / banner / redirect the surface's UI contract `states` block doesn't list. → Declare it first: the E2E author locks the `states` entry in `docs/ui-contract/<screen>.yaml` as part of authoring (the assertion surface is author-owned), then asserts the declared proof — never an undocumented element.
