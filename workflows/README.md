...
| **Coverage gate** | `runReviewSlice('test-coverage')` over the authored specs, looping to an `e2e-author` fix until the specs cover every AC + non-happy-path. No round cap — loops to APPROVE. Skipped on resume only when the newest `# E2E Coverage Gate` comment says APPROVE (ticked e2e boxes prove *authoring*, not passage — a run killed on a BLOCK re-gates). | `runReviewSlice()` + `agent()` |
...
- **`halt()`** flips `status:in-progress` → `status:need-attention` and posts a comment — the only path to a human. The outer `/loop` never recovers it; the user does. A review that can't set up its worktree (or otherwise crashes) returns `{ error }` and halts here, so an infra failure never spins the uncapped loop forever. Two progress guards bound the uncapped loops the same way: the **oscillation stall** (the same blocker — file + title fingerprint — surviving its own targeted fix for `STALL_ROUNDS` (3) consecutive rounds) and the **churn guard** (`CHURN_ROUNDS` (3) consecutive rounds each surfacing a NEW blocker on code *unchanged* since the prior round, per the round's `changedSincePaths` — under anchored re-review that is reviewer sampling noise, not defects). Genuine progress trips neither.
- **Resume.** A cold restart re-reads the checklist (durable: ticked `[x]` boxes = done tasks, skipped) in Prep, and the `Workflow` resume journal replays unchanged `agent()` prefixes. No handoff docs. **Review passage is resumed from the verdict comments, not the checklist:** every verdict comment is stamped with the reviewed branch-tip SHA (`**Reviewed tip:**`) plus an invisible `<!-- resume-state: ... -->` marker carrying the loop-guard streaks, and each looping review stage runs a one-shot read-only `reviewEntryAction` probe before its first review (the gate review's probe is hoisted to the Pass E2E phase so its result can skip that phase too). The probe drives four outcomes: the **coverage gate is skipped** on a durable APPROVE — ticked e2e boxes prove *authoring*, not passage, so a run killed on a BLOCK re-gates; **Pass E2E + Gate review are skipped** on a gate APPROVE whose SHA still equals the remote tip (straight to the idempotent AC-tick + PR; the quality pass is likewise skipped when its own comment already exists at that tip); a standing **BLOCK with no landed fix** — no commit and no fix-summary comment after it (a pushed commit is the only durable proof, since `setup-worktree.sh` hard-resets to `origin/<branch>`) — dispatches the fix straight away (`fix-first`) instead of re-running the fan-out only to reproduce the same BLOCK; and any BLOCK **re-seeds the oscillation + churn guards** from its resume-state marker, so a structurally stuck blocker can't evade the `STALL_ROUNDS` / `CHURN_ROUNDS` halts by being killed and relaunched.
## `runReviewSlice(scope, phaseTitle, sliceBranch, scopeManifest, tasks, reviewMode, roundCtx, guardIn)` — the inlined fan-out review
...
**Boundary:** it **posts the verdict comment and RETURNS the verdict object** (`{ verdict, publishError, blockers, findings, reviewedSha, changedSincePaths }`, or `{ error }` on infra failure) — it flips **no** label and opens **no** PR. The surrounding `implement-slice` phases own the lock, the fix loops, the AC-tick, and the terminal draft PR.
...
| **Fix** | A one-shot `reviewEntryAction` resume probe first: a durable `# Bug Fix Gate Review` APPROVE still at the remote fix-branch tip **skips Fix + Gate review entirely**, and a standing BLOCK with no landed fix routes the first dispatch to the review-feedback verb (the fix-first semantics, moved up from the gate). Then one `engineer` dispatch (`Fix bug #<n>`): write the regression test FIRST (RED, fails on pre-fix code), drive it GREEN, refactor, propagate the class-of-bug, push. A **completion check** (the bug-side analogue of Implement's checkbox verification — a bug has no checklist) then confirms a `Refs #<n>` regression test exists on the branch **and passes when run**, re-dispatching the engineer up to `STALL_ROUNDS` times before halting — a silently killed engineer can't carry a partial fix into a gate review whose reviewers read code but never execute the suite. | `agent({agentType: engineer})` + `agent()` |
| **Gate review** | `runReview('gate')` over the **gating dimensions only** (regression coverage / contract / security), looping to an `engineer` gating fix until APPROVE. No round cap. The `test-coverage` dimension enforces the regression test fails-before / passes-after. (No AC-tick — a bug has no acceptance-criteria ledger.) Skipped on resume when the Fix-phase probe found an APPROVE at the current tip; on a BLOCK the loop guards re-seed from the newest comment's `resume-state` marker. | `runReview()` + `agent()` |
| **Quality review** | `runReview('quality')` over the **code-quality axes only** (never block). **Bounded, not a loop:** one review/fix cycle (review → one `engineer` polish pass over the `Defer`/`Nit` findings) + one final re-review, whose residual debt is triaged into `kind:refactor` / `kind:enhancement` tracking issues. Skipped on resume when its own comment already exists at the reviewed tip (prevents duplicate debt triage). | `runReview()` + `agent()` |
| **PR** | Open the idempotent `merge:auto` draft PR (`Closes #<n>`) and release the `status:in-progress` lock. | `agent()` |

`runReview(phaseTitle, fixBranch, reviewMode, roundCtx, guardIn)` is the production-code-only port of `runReviewSlice()` — same Prep → fan-out → dedup → verify → compose → Publish pipeline, split by `reviewMode` exactly as `runReviewSlice`'s `'gate'` / `'quality'` modes, but with no `test-coverage` *scope* (the coverage gate is an `implement-slice`-only pre-implementation phase; for a bug, test-coverage is just one gating **dimension** over the fix diff). It posts a `# Bug Fix Gate Review` (gate mode) / `# Bug Fix Quality Review` (quality mode) comment on the bug issue — SHA-stamped and carrying the `resume-state` marker, same as the slice reviews — and returns the verdict. `halt()` flips `status:in-progress` → `status:need-attention`, the only path to a human.
