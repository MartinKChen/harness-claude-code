...
- **Resume.** A cold restart re-reads the checklist (durable: ticked `[x]` boxes = done tasks, skipped) in Prep, and the `Workflow` resume journal replays unchanged `agent()` prefixes. No handoff docs. **Review/fix resume probe:** each review loop (coverage gate, slice review) runs a one-shot read-only `reviewEntryAction` probe *before* its first review. If the branch already carries a standing **BLOCK** verdict that nothing has been done about — **no commit and no fix-summary comment landed on the branch after it** — the probe returns `fix-first`, so the workflow dispatches the fix straight away instead of re-running the fan-out only to reproduce the same BLOCK. A fix that *did* land (a commit after the BLOCK, even a partial one) → `review`: the re-review re-evaluates the current diff and catches whatever remains. Because every `setup-worktree.sh` hard-resets to `origin/<branch>`, a pushed commit is the only durable proof a fix landed — unpushed/local work is gone on relaunch, so "no commit after the BLOCK" is the correct trigger to (re)dispatch the fix.
...
**Scope `test-coverage`** runs only the Spec-phase `test-coverage` dimension over the **authored E2E specs**, pre-implementation: the usual "test files are out of scope" rule **inverts** (the specs are the deliverable), the verdict is BLOCK on **any** confirmed coverage gap (not just `I:H`), and Quality is skipped. Its closed AC set is **narrowed** to the **E2E-owned subset** — the union of `covers:` across the slice's `e2e` tasks, not the full slice AC set — so the gate never BLOCKs on a backend/frontend-owned AC (a ledger delta, token state, "no row created", a "the UI shows…" clause discharged by an API-level / RTL test) having "no E2E coverage". The full AC set is still gated by the `production-code` review, where every AC must be discharged at *its* owning layer.
...
| **Review** | A one-shot `reviewEntryAction` resume probe first (skip the re-review and dispatch the fix straight away when a standing BLOCK has no landed fix — see implement-slice's Resume note), then `runReview('production-code')` looping to an `engineer` fix (`Fix the review feedback on bug #<n>`) until APPROVE. No round cap. The `test-coverage` dimension enforces the regression test fails-before / passes-after. | `runReview()` + `agent()` |
