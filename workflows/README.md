...
| **Gate review** | `runReviewSlice('production-code', …, 'gate')` over the **gating dimensions only** (spec-compliance / contract / security), looping to an `engineer` fix-slice until APPROVE. No round cap. On APPROVE it ticks the AC checkboxes (the reviewer-verified gate). | `runReviewSlice()` + `agent()` |
...
## `runReviewSlice(scope, phaseTitle, sliceBranch, scopeManifest, tasks, reviewMode)` — the inlined fan-out review

Called by `implement-slice` for the pre-implementation E2E coverage gate (`scope:'test-coverage'`) and the **two** post-implementation review stages (`scope:'production-code'`, switched by `reviewMode`). It sets up a read-only worktree on the slice branch tip, isolates each review dimension onto its own `axis-reviewer` agent, and — when the `verifyLenses` arg is on (default OFF) — adversarially verifies every finding before it counts.
...
**Boundary:** it **posts the verdict comment and RETURNS the verdict object** (`{ verdict, publishError, blockers, findings }`, or `{ error }` on infra failure) — it flips **no** label and opens **no** PR. The surrounding `implement-slice` phases own the lock, the fix loops, the AC-tick, and the terminal draft PR.
...
| **Fan-out** | Fan out the dimension set selected by `(scope, reviewMode)` — gating dims in `'gate'`, quality axes in `'quality'`, the lone `test-coverage` dim in coverage scope — one `axis-reviewer` agent each → **dedup** → **verify**. | One axis per agent, clean context. |
