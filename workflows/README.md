...
- **No convergence cap.** Every fix loop (coverage gate, implement re-dispatch, slice review) runs until it reaches confidence to pass — review `APPROVE`, or every task ticked `[x]` — rather than halting after a fixed number of rounds. The review fan-out is tuned for aggressive recall, and its adversarial verify phase is what keeps the loops convergent: only a finding that survives refutation holds a gate open, and `production-code` review blocks on `I:H` alone, so once the real blockers are fixed the loop ends. The deleted engineer budget gate's "bound the context" role is covered by the planner's ≤3-tasks-per-group size cap + small-task scoping.
...
Called by `implement-slice` for **both** the pre-implementation E2E coverage gate (`scope:'test-coverage'`) and the post-implementation slice review (`scope:'production-code'`). It sets up a read-only worktree on the slice branch tip, isolates each review dimension onto its own `axis-reviewer` agent, and adversarially verifies every finding.
...
| **Verify** | Per finding, 3 independent skeptic lenses (`correctness`, `context`, `severity`) read the real code and try to **refute** it; survives on a majority. These run on the default workflow agent (refute ≠ find), not `axis-reviewer`. | Kills false `I:H` before it triggers a fix cycle. |
