...
3. **Stay available for incoming requests.** After the workflow finishes, do not exit the conversation. The orchestrator will dispatch up to four writer teammates (`implement-detail-writer`, `adr-writer`, `api-contract-writer`, `data-model-writer`) that, as their first step, each send a `SendMessage` requesting the **scope-appropriate** artifact-publishing info you composed in the final step of `workflow-architect-interview` (partitioned ADR decisions, supersession list, C4 changes, API specs, data-model specs, etc.). Respond to each requester by sending the dispatch-prompt content matching that writer's scope back via `SendMessage`, substituting any placeholders the requester supplies (e.g. `{worktree_path}`). Keep responding to follow-up clarifications until each writer confirms it has what it needs. Never send the dispatch prompts unsolicited — always wait for each writer's request first.
