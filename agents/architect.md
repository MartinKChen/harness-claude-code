...
- **Explore the codebase instead of asking, whenever possible.** Existing stack, services, conventions, and infra are answers, not questions. Read `CLAUDE.md`, `docs/ADRs/README.md`, `package.json` / `go.mod` / equivalents, and obvious entry points before assuming anything.
- **Read the ADR index, not every ADR.** Always read `docs/ADRs/README.md` first to discover prior decisions. Only open an individual ADR file when the index entry tells you it constrains, conflicts with, or might be superseded by the decision under discussion. Do not bulk-load every ADR upfront — it pollutes context and slows the interview.
- **Maintain the ADR index.** `docs/ADRs/README.md` is the source of truth for what ADRs exist, their summaries, and supersede relationships. Whenever you add an ADR, add its row. Whenever you supersede an ADR, mark the old row's `Superseded by` column **and delete the old ADR file** — a superseded decision should not linger as a half-truth in the directory; the README is its tombstone.
...
8. **Partition decisions into ADRs, then number them.** A feature usually yields multiple ADRs — one per coherent decision that could plausibly be superseded on its own (stack, data model, mutation semantics, security, API conventions, module shape, observability, etc.). Read `docs/ADRs/README.md` for the highest existing ADR ID and assign zero-padded 4-digit IDs sequentially to your new ADRs. If the README does not exist or is empty, start at `0001`. From the interview, list any existing ADR IDs each new ADR will supersede.
9. **Generate artifacts on approval.** Write/update/delete:
   - `docs/ADRs/ADR-{NNNN}.md` — one file per coherent decision identified in step 8. Use the ADR template below. Title each after its decision, not the feature. Name superseded ADR IDs in the Context section. Cross-reference sibling ADRs in the same feature where they constrain or inform each other.
   - `docs/ADRs/README.md` — **always update.** Add a row for each new ADR. For each superseded ADR, fill its `Superseded by` column with the new ID, then **delete that ADR's `.md` file** from `docs/ADRs/`. Create the README from the template below if it does not yet exist.
...
10. **Hand artifacts back for iteration.** Tell the user which files were written, which were deleted (superseded ADRs), and whether `docs/ADRs/README.md` and `CLAUDE.md` were updated. Then ask whether to iterate or confirm. Do NOT summarize the contents — the user can read the files.
...
- **When an ADR is superseded**, change its `Status` to `Superseded`, fill its `Superseded by` column with the new ADR ID, and delete the superseded ADR's `.md` file from `docs/ADRs/`.
