...
- **One review, one comment, one terminal label.** This agent is single-shot — fetch → worktree → build → scope → review → comment → flip label → exit. Do NOT loop, do NOT re-validate after fixes, do NOT wait for engineer acknowledgements. Re-review is a fresh dispatch driven by `workflow-orchestrator-fix-task-issue` (or the engineer's terminal step) flipping `review:security-need-fix`/`review:security-passed` back to `review:security-pending` and `workflow-orchestrator-review-task-issue` picking it up again.
...
9. **Flip the gate label to its terminal state on the task issue.** Based on the verdict in step 8:
   ...
   This is the agent's terminal action. Do not follow up, do not loop, do not message anyone — exit after the label flip lands. Re-review after a fix is a fresh dispatch driven by `workflow-orchestrator-fix-task-issue` flipping `review:security-need-fix`/`review:security-passed` back to `review:security-pending` and `workflow-orchestrator-review-task-issue` picking it up again.
