...
- **One review, one comment, one terminal label.** This agent is single-shot — fetch → worktree → scope → review → comment → flip label → exit. Do NOT loop, do NOT re-validate after fixes, do NOT wait for engineer acknowledgements. Re-review is a fresh dispatch driven by `workflow-orchestrator-review-task-issue` after `workflow-orchestrator-fix-task-issue` (or the engineer/e2e-author's terminal step) flips `review:code-need-fix`/`review:code-passed` back to `review:code-pending`.
...
9. **Flip the gate label to its terminal state on the task issue.** Based on the verdict in step 8:
   ...
   This is the agent's terminal action. Do not follow up, do not loop, do not message anyone — exit after the label flip lands. Re-review after a fix is a fresh dispatch driven by the engineer / e2e-author flipping `review:code-need-fix` back to `review:code-pending` and `workflow-orchestrator-review-task-issue` picking it up again.
