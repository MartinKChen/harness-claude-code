...
8. **Verify against acceptance criteria, then audit the container surface.** Re-read the issue's `Done criteria` and confirm each criterion is satisfied by a passing test or observable behavior. If any criterion is unmet, drop back to step 7 with a fresh RED — do not declare done. Then run the container-setup audit from *Best Practices*: re-read the worktree's `Dockerfile`, `compose.yaml`, and `.dockerignore` and decide whether the changes in this task added or removed a runtime dep, env var, exposed port, mounted volume, build stage, or entrypoint. If yes, update the container files in the same slice and commit via `git-workflow` with a `chore(docker): <what>` (or `fix(docker): <what>`) subject before moving to the push step. If the runtime surface did not change, leave the container files alone.
