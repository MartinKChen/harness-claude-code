...
- **Mutation testing is a diff-scoped PR-validation job, never whole-repo and never in the push path.** PR validation runs a per-stack mutation job (`bash scripts/mutation.sh "origin/${{ github.base_ref }}"` — Stryker `--since` / mutmut over the branch diff) that gates on a surviving mutant on a changed line and uploads the surviving-mutant report as an artifact (the report the test-coverage reviewer consumes). It is its OWN job depending on the per-stack checks — keep it out of `scripts/ci-checks.sh` and the local pre-push hook, which must stay fast: mutation runs the suite once per mutant, so only the diff-scope keeps it CI-affordable. Checkout with `fetch-depth: 0` so the diff base resolves.
