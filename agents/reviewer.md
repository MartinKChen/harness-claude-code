...
| `pattern-reviewer-contract` | Code gate, when touched paths include API routes OR ORM models AND a sibling contract file exists under `docs/product-requirement-document/<feature>/api-contract/` or `data-model/`. Audits conformance to the contract (path / verb / status / body / envelope; table / columns / constraint names / relationships). | Yes when triggered |
