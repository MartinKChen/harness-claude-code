...
- **Enhancement and bug-fix lifecycles.** Now driven by the unified `/ship` command (superset of `/implement-feature`, which stays as a feature-only fallback). **Bugs** are fully wired: a read-only `analyze` engineer reproduces (browser MCP preferred, Playwright fallback, stack booted either way) and posts a `# Bug Analysis` comment → human approves the approach → the lighter `fix-bug.mjs` workflow writes the regression test first, drives it green, reviews, and opens a PR. **Enhancements** route through the existing `implement-slice.mjs` (an enhancement is a single feature-shaped issue), so the finder + command support them today; still pending is the enhancement *issue template* + branch-linking helper so one can be filed without `/create-issues`. Both lanes run repo-wide (no milestone needed) via `/ship` with no argument.
