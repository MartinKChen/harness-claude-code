...
| `pattern-reviewer-backend-standard` | Backend audit — unvalidated input, missing rate limits, unbounded queries, N+1, missing timeouts, error-message leakage, error-envelope conformance, idempotency wiring, atomic mutations, `/healthz` shape, log redaction, `.env.example` lockstep, locked deps. |
