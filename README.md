...
| `/workflow-orchestrator-review-task-issue` | Scans in-progress tasks carrying `review:code-pending` or `review:security-pending`, flips the pending gate(s) to `-running`, and dispatches the matching reviewer (`code-reviewer` / `security-reviewer`) in the background. Reviews are scoped to the task issue, not the slice PR. |
