...
- **The expensive unit is a slice.** Per `implement-slice` run: E2E authoring (only when the slice has an `e2e` task) + a coverage-gate fan-out + implementation + the pass-E2E loop + a production-code review fan-out of up to ~14 `pattern-reviewer-*` dimensions, optionally confirmed by a 3-lens adversarial verify (batched ≤10 findings/agent) — **opt-in via the `verifyLenses` arg / `$HCC_VERIFY_LENSES`, default OFF** (without it the dimension reviewer's own severity stands; the self-review pass is not run). Fix loops are **bounded by progress, not a fixed round cap** — they halt to a human after `STALL_ROUNDS` (3) consecutive non-progressing rounds, so a pathological slice escalates rather than burning unboundedly.
...
| `runReviewSlice()` / `runReview()` | inlined function inside each workflow | Fans review out across isolated `pattern-reviewer-*` dimensions — one `axis-reviewer` agent each — dedups overlapping findings, **optionally** adversarially verifies each through three skeptic lenses (batched per dimension, ≤10 findings per agent, so dispatch scales with dimensions × chunks rather than one agent per finding — the cross-lens majority vote is unchanged) — this verify pass is **opt-in via the `verifyLenses` arg (default OFF, since it is itself a self-review); with it off the reviewer's own severity stands** — then **posts one `# Slice Review` / `# E2E Coverage Gate` / `# Bug Fix Review` comment and RETURNS the verdict** — flips no label, opens no PR (the surrounding phases own those). Scopes: `test-coverage` (gate the authored E2E specs pre-implementation against the slice AC + non-happy-paths; spec dims only; BLOCK on any gap) and `production-code` (the two-phase walk against implemented code; BLOCK on a surviving `I:H` from a **gating** dimension — spec-compliance / contract / security — while a high-impact **code-quality** finding is recorded as deferred debt for a periodic quality sweep and never blocks). **Both scopes are bounded by the Scope Manifest** (rendered into every dimension agent's *and* every verifier's context) plus a **per-task discharge ledger** (each task's owning layer + the `covers:`/`scenario:` it discharges there): the coverage gate covers exactly the manifest's `acIds` + their Gherkin (a behavior with no AC id is not a gap); the production review requires every finding to ground in a declared `acId` or a touched-path rule (no prose-synthesized ACs), judges each task at its **owning layer** (a backend invariant is proven at the backend layer, never demanded through E2E), and treats a missing test for unchanged behavior as at most a Deferred/Nit, never a blocker. On a `production-code` APPROVE the AC checkboxes are ticked — the verified gate — then a **single code-quality polish cycle** (one engineer fix over the non-gating findings + one re-review) runs and the **residual** non-gating findings are **triaged into tracking issues — one per review dimension, routed by kind**: `non-functional` → `kind:enhancement` (feature-shaped, earns full E2E/integration coverage), every other dimension → `kind:refactor` (behavior-preserving — a no-`e2e` `## Tasks` checklist so `implement-slice` skips all E2E machinery; unit-test new seams only). Both land at `status:ready-to-review` (the human gate), deduped against open issues of that kind, so quality debt is recorded for `/ship` rather than blocking the slice. `fix-bug.mjs` duplicates the production-code path inline — including the polish cycle + triage (workflow scripts are self-contained — no shared import). |
...
Scoring (`severity → Impact`, `(Impact, Effort) → Fix/Defer/Nit/Drop`, `full verdict = BLOCK iff a gating-dimension (spec / contract / security) I:H survives` — code-quality `I:H` is deferred debt) runs as plain deterministic JS, not an LLM step, so it is identical across runs.
...
4. **Inner cycle** (one workflow): Prep derives the Scope Manifest → `e2e-author` writes Playwright specs → coverage-gate fan-out (BLOCK → e2e-author fix loop) → engineers implement the tasks via outside-in TDD → pass-E2E loop (diagnose → per-group fix until green) → production-code review fan-out (BLOCK on a gating-dimension — spec / contract / security — `I:H` → fix loop; code-quality findings are deferred debt) → on the gating APPROVE one **code-quality polish cycle** runs (a single engineer fix over the non-gating findings, then one re-review) → the AC checkboxes are ticked, the **residual** code-quality findings are **triaged into tracking issues — one per review dimension** (`non-functional` → `kind:enhancement`, the rest → behavior-preserving `kind:refactor`; both `status:ready-to-review`, deduped — for the `/ship` maintenance lane), and a `merge:auto` **draft PR** opens with `Closes #42`. The slice lock is released.
