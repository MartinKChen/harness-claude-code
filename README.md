...
| `/workflow-orchestrator-review-task-issue` | Scans in-progress tasks carrying `review:pending`, flips to `review:running`, and dispatches the `reviewer` sub-agent in the background — one per task. The reviewer picks the pattern-skill set from the task's `type:*` label and covers both code-quality and security concerns in one pass. Reviews are scoped to the task issue, not the slice PR. |
...
| `/workflow-orchestrator-close-task-issue` | Closes every in-progress task whose review gate has reached `review:passed`. |
...
| `e2e-author` | sonnet | Authors and extends Playwright E2E tests for a single task issue. Self-driven from an issue ID — sets up its own slice-scoped worktree rebased onto main, writes tests, smoke-runs them, commits to the slice branch, pushes, and flips `review:pending` on the task. PR creation is owned entirely by the `/workflow-orchestrator-create-draft-pr` skill once every task sub-issue under a slice has closed. The full Playwright suite is validated by a GitHub Actions workflow on the slice PR. |
