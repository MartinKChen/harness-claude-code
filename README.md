...
| `pattern-reviewer-contract` | Conformance audit — every API endpoint matches its `docs/product-requirement-document/<feature>/api-contract/<entity>.md` (path / verb / status / body / envelope / idempotency / rate-limit), and every ORM model matches its `data-model/<entity>.md` (table / columns / constraint names / relationships). |
