...
1. **Author E2E first** — Playwright specs are written from the slice's Gherkin before implementation, and gated by a coverage review.
2. **Outside-in TDD** — the engineer grows each module red → green → refactor, one behavior per commit, against the locked contract.
